...
import BattleShip.*;
class modeThree extends JFrame implements MouseListener {
    int gridNum=8;
...
//                    checkerBoard2.draw_Board();
                    ...
                    promptLabel=new Label("PC(AI) Setting......");
...
//                    System.out.println(" ("+i+" "+j+")");
    ...
    public modeThree(){
        this.setTitle("JFrameUI.modeThree");
        ...
        checkerBoard1.draw_Board();
        checkerBoard2=new CheckerBoard("PC(AI)",lx2,ly2,this);
        checkerBoard2.draw_Board();
    ...
    int Get_neighbors(int x,int y){
        ...
        for(int i=-1;i<1;i++)
        {
            for(int j=-1;j<1;j++)
            {
                int xx=i+x;
                int yy=j+y;
                if(xx>=0&&xx<gridNum&&yy>=0&&yy<gridNum)
                {
                    if(checkerBoard1.getGrids()[xx][yy].getType()== GridType.HIP_SHIP)num++;
                }
            }
        }
        return num;
    ...
    int[] getloca_x_y(){
        int res[]=new int[2];
        int max=0;
        int point[][]=new int[gridNum][gridNum];
        for(int i=0;i<gridNum;i++)
        {
            for(int j=0;j<gridNum;j++)
            {
                point[i][j]=Get_neighbors(i,j);
                if(point[i][j]>=3)point[i][j]=-point[i][j];
                if(point[i][j]>max)max=point[i][j];
            }
        }
        int aix,aiy;
        while(true)
        {
            ...
            aix=(int)(Math.random()*gridNum);
            aiy=(int)(Math.random()*gridNum);
            if(point[aix][aiy]==max&&checkerBoard1.getGrids()[aix][aiy].getType()== GridType.SHIP||checkerBoard1.getGrids()[aix][aiy].getType()== GridType.SEA)break;
        }
        res[0]=aix;
        res[1]=aiy;
        return res;
    ...
    void pcPlay(){
        ...
        aix=f[0];
        aiy=f[1];
        if(checkerBoard1.getGrids()[aix][aiy].getType()== GridType.SHIP)
        {
            hitshipMusic.play();
            checkerBoard1.Set_Grid_according_to_location(aix,aiy,new Grid(GridType.HIP_SHIP));
//                                        System.out.println("p1Hit");
        ...
        {
            checkerBoard1.Set_Grid_according_to_location(aix,aiy,new Grid(GridType.HIP_SEA));
            drawPrompt(checkerBoard1);
